...
describe('1896 Educational Series resource page', () => {
  ...
  it('keeps Article JSON-LD and does not publish an example note', () => {
    ...
    assert.match(bodyEs, /aún no añaden un ejemplar/);
    assert.match(bodyEn, /serial B3207078/);
    assert.match(bodyEn, /do not yet add an example/);
    ...
    assert.doesNotMatch(bodyEn, /will be published when the scan is in hand/);
