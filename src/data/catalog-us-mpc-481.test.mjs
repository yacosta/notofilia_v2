...
describe('US Series 481 MPC 5 cents D02536728D', () => {
  ...
  it('keeps thin ES and EN piece routes on the MPC note layout', () => {
    ...
    assert.match(seriesPage, /frame="fit"/);
    const seriesHero = readFileSync(new URL('../components/catalog/SeriesHero.astro', import.meta.url), 'utf8');
    assert.match(seriesHero, /isFit \? `\$\{src\} \$\{width\}w`/);
    assert.match(seriesHero, /imgSizes = isFit \? `\$\{width\}px`/);
