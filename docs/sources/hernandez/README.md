# Pedro Pablo Hernández, *Monedas y Billetes de Colombia* (8ª ed., 2023)

Research extract for Notofilia. **Do not republish plates, price columns, or dealer advertisements** from this file.

## File

| File | Role |
| --- | --- |
| `monedas-y-billetes-colombia-2023-8ed.txt` | OCR of the 8th edition (dirty scan; prices and ads are in the extract — **omit them from the site**). |

## Bibliographic record

- **Title:** *Monedas y Billetes de Colombia* — subtitle *Cien Años Banco de la República 1923–2023*.
- **Author / editor:** Pedro Pablo Hernández (compiler); **8th edition, 2023**.
- **ISBN (print, from title verso):** 978-958-49-7741-0.
- **Public citation on the site:** Numista literature **[L100183](https://en.numista.com/catalogue/literature.php?id=100183)** (Hernández, *Monedas y Billetes de Colombia*). Do **not** cite this repo path.
- **Cover series:** “Cien Años Banco de la República” is the 1923–2023 centennial frame. It does **not** make this an official Banco de la República publication.
- **Scope:** the 8th edition covers coins and banknotes **before 1923 and through 2023** (independence, banca libre, Banco Nacional, and BanRep types). It is complementary to González White 2019 for BanRep 1923–2019; do not treat Hernández as pre-1923 only.

## Numbering

- This edition uses **Cód.** (catalogue codes) on coins and notes (e.g. **Cód. 11** Santa Marta 1820 ¼ real; **Cód. 1109** Banco de Riohacha 5 pesos 1 Jan 1883; **Cód. 1134** Banco Hipotecario 5 pesos ABNC).
- Older Notofilia copy citing **Hernández 91 / 95 / 96 / 199 / 201** follows **Numista** concordance to an earlier Hernández listing. Do **not** overwrite those H# with 2023 Cód. unless Numista is updated.

## What to take / what to leave

**Take (facts, already used or available for copy):**

- Independence paper: Hernández lists **2 reales 1812** at **Mompós** as the first Colombian notes in his catalogue (unique known in his listing), then **1 real 1813** signed Germán Gutiérrez de Piñeres; ordinary paper; 100 reales of this paper traded for 16; possible **1814**. Keep Banrepcultural’s **1813** 1-real lead in parallel — do not drop it.
- Gran Colombia remainders: **never issued**, unsigned, **Peter Maverick** (New York); 6 / 12 / 18 / 30 pesos in **sheets of six**; salt at Zipaquirá, Nemocón, Tausa; Bolívar vs Santander in the vignettes.
- **1922 Junta de Conversión** (Ley 5, Decreto 165, escritura 206 Notaría 3ª): treasury bonds overstamped before BanRep **23 July 1923**.
- Banco Hipotecario: founded Bogotá **1880s**, **liquidated before 1914**; 5 pesos ABNC seated woman / horses = **Cód. 1134**. Distinguish Banco de la Mutualidad (1914) and Banco del Pacífico.
- Banco de Riohacha: founded **1885**, liquidated **1888**; circulating **5 pesos 1 Jan 1883** Bolívar centre = **Cód. 1109**. Notofilia’s piece is **ABNCo proofs**, not that circulating row.
- Santa Marta **1820** royalist copper ¼ real **SM** = **Cód. 11** (already on the coin record).
- Specimen / prueba / **mariposa** (miscut) annex definitions — glossary only, no plates.
- BanRep **Un Peso Oro** (1923–1977, then nickel coins): Imprenta de Billetes type **Cód. 45–81**. Ordinary **7 August 1973** = **Cód. 76** (Numista Hernández 76 / P# 404e; year as «1.973»); **Cód. 77** «7 Corto en el mes»; **Cód. 78** asterisk replacement. Last 1-peso date **1 January 1977** = **Cód. 81**. Annex 2 replacement interval for 7 August 1973 asterisk: **00105759–00541728** (not a holding).

**Leave:**

- All **$** / **US $** price columns and dealer ads (ASOCOPA, etc.).
- Plate serials that are **not** Notofilia holdings (e.g. 20.000 pesos **29 Aug 2017** plate **AC53513901** / series **AE** **675NF** — collection note is **AF51355141**, González White **BG# NF20-6** series **AF**).
- Invented printage from a Cód. row.

## Current Notofilia holdings (lookup)

Confirm in the OCR, then on Numista L100183, before changing published copy. Do **not** republish `$` / `US $` columns or plates.

| Holding | Where in this book |
| --- | --- |
| 1 peso oro, 7 August 1973, Imprenta de Billetes, no series letter (Pick 404e), serial **26530968** | **Un Peso Oro**, ordinary **Cód. 76** (Numista Hernández 76; year as «1.973»). Distinct from **Cód. 77** «7 Corto en el mes» and **Cód. 78** asterisk replacement. Annex 2 asterisk interval **00105759–00541728** does not include this serial. Type **Cód. 45–81**; last 1-peso date **Cód. 81** (1 January 1977), then nickel coins. Complementary to González White 2019 p. 41 (do not guess a BG# from that OCR). N# 205723 had no earlier H#; publish the 2023 Cód. as Hernández’s number. |
| 5 pesos oro replacement, 1 April 1979, IBB asterisk, serial **00141180** (Pick 406f) | **Cinco Pesos Oro** **Cód. 156** (Numista Hernández 156), distinct from circulating **Cód. 155** of the same date (P# 406f / TBB B949o). Neighbouring 5-peso rows **Cód. 150 / 153 / 160** are labelled «Rep., Asterisco en el anverso». Annex 2 (p. 322) asterisk interval for 1 April 1979: **00100731–00135113** — **does not** include 00141180. Do not republish price columns. Complementary to González White 2019 p. 60 (do not guess a BG# from that OCR). |
| 20 pesos oro, 1 January 1983, Imprenta de Billetes, serial **056462955** (Pick 409d) | **Veinte Pesos Oro** ordinary **Cód. 243** (Numista Hernández 243 / TBB B951l). Distinct from **Cód. 244** «Rep., Asterisco en el anverso». Annex 2 asterisk interval for 1 January 1983: **00102221–00171449** does not include this serial. Type ended in 1983, replaced by copper 20-peso coins. Complementary to González White 2019 pp. 73–74 (do not guess a BG# from that OCR). |

## Related

- González White 7ª ed. 2019: `docs/sources/gonzalez-white/` (`41-gonzalez-white.mdc`).
- BanRep collection booklet 1996: `docs/sources/banrep/coleccion-numismatica-museo-casa-de-moneda.pdf`.
